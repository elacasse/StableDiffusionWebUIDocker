...
RUN DEBIAN_FRONTEND=noninteractive apt-get install -yq --no-install-recommends google-perftools nano bc wget
RUN apt-get clean

FROM step-dependencies AS step-user

RUN useradd -m stablediffusion
RUN adduser stablediffusion root
RUN adduser stablediffusion sudo
RUN echo "stablediffusion ALL=(ALL) NOPASSWD: ALL" >> /etc/sudoers
...
COPY --chown=stablediffusion:stablediffusion --chmod=755 init.sh .

VOLUME /home/stablediffusion/stable-diffusion-webui
